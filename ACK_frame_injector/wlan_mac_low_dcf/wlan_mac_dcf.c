...
//volatile static u8  v_ap[MAC_ADDR_LEN] = { 136,54,108,4,74,216 };
volatile static u8  example[MAC_ADDR_LEN] = { 64,216,85,4,36,50 };
volatile static u8  example_1[MAC_ADDR_LEN] = { 64,216,85,4,36,57 };
...
//	wlan_mac_set_postRx_timer1((gl_mac_timing_values.t_sifs*100)-(TX_PHY_DLY_100NSEC));
...
u32 frame_receive(u8 rx_pkt_buf, phy_rx_details_t* phy_details) {
    ...
    u8                  tx_mcs;
    u16                 cts_duration;
    u8                  unicast_to_me, to_multicast, unicast_to_me_1, unicast_to_me_2;
    u16                 rssi;
    u8                  lna_gain;
    u8                  active_rx_ant;
    u32                 rx_filter;
...
//        case RX_ACTIVE_ANTB:  tx_ant_mask |= 0x2;  break;
    ...
    unicast_to_me_1 = wlan_addr_eq(rx_header->address_1, v_ap);
    unicast_to_me_2 = wlan_addr_eq(rx_header->address_1, v_ap);
    ...
    if(unicast_to_me_1 && !WLAN_IS_CTRL_FRAME(rx_header)) {
...
//        tx_length = wlan_create_ack_frame((void*)(TX_PKT_BUF_TO_ADDR(TX_PKT_BUF_ACK_CTS) + PHY_TX_PKT_BUF_MPDU_OFFSET), (u8 *) example_1);
